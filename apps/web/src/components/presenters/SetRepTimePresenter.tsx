...
interface SetRepTimePresenterProps {
    metric: any;
    onLog: (set: number, rep: number, time: number) => Promise<void>;
    isLoading: boolean;
}

export function SetRepTimePresenter({ metric, onLog, isLoading }: SetRepTimePresenterProps) {
    const [set, setSet] = React.useState("");
    const [rep, setRep] = React.useState("");
    const [time, setTime] = React.useState("");

    const handleLog = async () => {
        if (set === "" || rep === "" || time === "") return;
        await onLog(Number(set), Number(rep), Number(time));
        setSet("");
        setRep("");
        setTime("");
    };
        ...
        <div className="flex flex-col gap-4 w-full h-full p-6 bg-black/20 rounded-2xl border border-white/5 relative shadow-inner">
            ...
            <div className="flex flex-col gap-4 mt-auto">
                ...
                <Button
                    ...
                    onClick={handleLog}
                    disabled={isLoading || set === "" || rep === "" || time === ""}
                >
